...
final class XingSeeker implements Mp3Extractor.Seeker {
    ...
    public static XingSeeker create(MpegAudioHeader mpegAudioHeader, ParsableByteArray parsableByteArray, long j, long j2) {
        ...
        long scaleLargeTimestamp = Util.scaleLargeTimestamp(readUnsignedIntToInt, i * C1167C.MICROS_PER_SECOND, i2);
        if ((readInt & 6) != 6) {
    ...
    public long getPosition(long j) {
        ...
        float f = (j * 100.0f) / this.durationUs;
        if (f <= 0.0f) {
            r0 = 0.0f;
        } else if (f < 100.0f) {
            int i = (int) f;
            float f2 = i != 0 ? this.tableOfContents[i - 1] : 0.0f;
            r0 = (((i < 99 ? this.tableOfContents[i] : 256.0f) - f2) * (f - i)) + f2;
        }
    ...
    public long getTimeUs(long j) {
        ...
        return timeUsForTocPosition + ((binarySearchFloor == 99 ? 256L : this.tableOfContents[binarySearchFloor]) == j2 ? 0L : (long) (((getTimeUsForTocPosition(binarySearchFloor + 1) - timeUsForTocPosition) * (d - j2)) / (r0 - j2)));
    }
